...
- [x] **DATA-05**: Cache `m4.cgi&hist=1` HTML for every class-2 Fair Haven parcel (~2,061). Each response content-validated for the requested block AND lot integers. Cache layout: `data/raw/oprs_prc/<pams_pin>/m4.html`.
- [x] **DATA-06**: For every sale ssi enumerated from each parcel's m4, cache the corresponding `sr.cgi?ssi=N` HTML at `data/raw/oprs_prc/<pams_pin>/sr_<ssi>.html`. Empty-template responses (legitimate "no detail on record") get a `.no_sale` marker file rather than the empty HTML.
